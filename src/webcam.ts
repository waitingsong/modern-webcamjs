...
const modInited: boolean = false     // module initialized
...
export const Webcam: Webcam = <Webcam> (config) => {
...
init.fn._set = function (sconfig) {
  ...
  if (sconfigOri) {
    ...
    const defaults = { ...cam.streamConfig, ...inst.config }
...
init.fn.prepare_snap_opts = function (opts): SnapParams {
  ...
  if (typeof opts === 'undefined' || !opts) {
    ...
    sopts = <SnapParams> { ...(sconfig ? sconfig : {}), ...opts }
  }
  else {
    sopts = <SnapParams> inst.get_stream_config(sidx)
  }
